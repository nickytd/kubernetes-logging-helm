...
openssl pkcs8 -inform PEM -outform PEM -in $BASEDIR/elk-rest/elk-rest-key-temp.pem \
  -topk8 -nocrypt -v1 PBE-SHA1-3DES -out $BASEDIR/elk-rest/elk-rest-key.pem
openssl req -new -key $BASEDIR/elk-rest/elk-rest-key.pem -out $BASEDIR/elk-rest/elk-rest.csr \
  -batch -verbose -config $BASEDIR/elk-rest/elk-rest.conf -extensions req_ext
openssl x509 -req -days 365 -in $BASEDIR/elk-rest/elk-rest.csr -CA $BASEDIR/ca/root-ca/root-ca.pem \
  -CAkey $BASEDIR/ca/root-ca/root-ca-key.pem -out $BASEDIR/elk-rest/elk-rest.pem \
  -extensions req_ext -extfile $BASEDIR/elk-rest/elk-rest.conf
openssl x509 -subject -nameopt RFC2253 -noout -in $BASEDIR/elk-rest/elk-rest.pem
openssl pkcs12 -export -name elk-rest -out $BASEDIR/elk-rest/elk-rest.p12 \
  -inkey $BASEDIR/elk-rest/elk-rest-key.pem -passout "pass:92cdf688aac64d17b230" \
  -in $BASEDIR/elk-rest/elk-rest.pem -CAfile $BASEDIR/ca/root-ca/root-ca-key.pem
